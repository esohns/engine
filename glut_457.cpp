...
void
engine_glut_457_reshape (int width_in, int height_in)
{
...
void
engine_glut_457_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_457_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_457_CBData*> (glutGetWindowData ());
...
void
engine_glut_457_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_457_CBData* cb_data_p =
...
void
engine_glut_457_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_457_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_457_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_457_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_457_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_457_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_457_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_457_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_457_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_457_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_457_timer,
                 v);
}

void
engine_glut_457_draw (void)
{
  struct Engine_OpenGL_GLUT_457_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_457_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_457_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_457_DEFAULT_HEIGHT));
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->mouseLoc,
                      ...
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_457_DEFAULT_HEIGHT, ENGINE_GLUT_457_DEFAULT_HEIGHT, 0)));

  glProgramUniform1i (cb_data_p->shader.id_, cb_data_p->pointerCountLoc,
                      static_cast<GLint> (cb_data_p->mouseLMBPressed ? 1 : 0));

  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_457_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_457_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_457_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_457_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_457_idle);
  else
    glutIdleFunc (NULL);
}
