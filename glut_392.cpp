...
void
engine_glut_392_reshape (int width_in, int height_in)
{
...
void
engine_glut_392_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_392_CBData*> (glutGetWindowData ());
...
void
engine_glut_392_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
...
void
engine_glut_392_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_392_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_392_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_392_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_392_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_392_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  if (cb_data_p->mouse0Clicked)
  {
    ...
    cb_data_p->viewY =
      static_cast<float> (Common_GL_Tools::map (y, 0, ENGINE_GLUT_392_DEFAULT_HEIGHT - 1, ENGINE_GLUT_392_DEFAULT_HEIGHT - 1, 0));
  } // end IF
}

void
engine_glut_392_timer (int v)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_392_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_392_timer,
                 v);
}

void
engine_glut_392_draw (void)
{
  struct Engine_OpenGL_GLUT_392_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_392_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glProgramUniform2f (cb_data_p->programId, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_392_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_392_DEFAULT_HEIGHT));
...
#error missing implementation, aborting
  ...
  glProgramUniform1i (cb_data_p->programId, cb_data_p->clickedLoc,
                      static_cast<GLint> (cb_data_p->mouse0Clicked ? 1 : 0));

  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_392_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_392_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_392_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_392_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_392_idle);
  else
    glutIdleFunc (NULL);
}
