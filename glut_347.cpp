...
void
engine_glut_347_reshape (int width_in, int height_in)
{
...
void
engine_glut_347_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_347_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_347_CBData*> (glutGetWindowData ());
...
void
engine_glut_347_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_347_CBData* cb_data_p =
...
void
engine_glut_347_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_347_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_347_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_347_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_347_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_347_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_347_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_347_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_347_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_347_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_347_timer,
                 v);
}

void
engine_glut_347_draw (void)
{
  struct Engine_OpenGL_GLUT_347_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_347_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glProgramUniform2f (cb_data_p->programId, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_347_DEFAULT_WIDTH) * 1.5f,
                      static_cast<GLfloat> (ENGINE_GLUT_347_DEFAULT_HEIGHT) * 1.5f);
...
#error missing implementation, aborting
  ...
  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_347_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_347_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_347_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_347_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_347_idle);
  else
    glutIdleFunc (NULL);
}
