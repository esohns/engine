...
march (vec3 ro, vec3 rd)
{
  ...
  if (stp)
    md = smoothstep(EDGE_SIZE-SMOOTH, EDGE_SIZE+0.01, md);
  else
    md = 1.;
...
main ()
{
  ...
  vec2 mo = iMouse.xy/iResolution*2.-1.;
  ...
  col  = mix(col,bg,smoothstep(30.,40.,rz.x));
  col *= mix(rz.z,1.,smoothstep(30.,40.,rz.y));
