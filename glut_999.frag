...
float
hftracing (vec3 ori, vec3 dir, out vec3 p, out float t)
{
  float d = 0.0;
  t = 0.0;
...
float
volume_tracing (vec3 ori, vec3 dir, float maxt)
{
  float d = 0.0;
  float t = 0.0;
  float count = 0.0;
...
vec3
terr_color (vec3 p, vec3 n, vec3 eye, vec3 dist)
{
...
main ()
{
  ...
  vec3 bn = texture(iChannel0, gl_FragCoord.xy / texSize + iTime * 0.2).xyz;
  vec2 uv = (gl_FragCoord.xy + (bn.xy * 2.0 - 1.0)/3.0) / iResolution;
  uv = uv * 2.0 - 1.0;
