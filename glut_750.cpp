...
void
engine_glut_750_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_750_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_750_CBData*> (glutGetWindowData ());
...
void
engine_glut_750_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
...
void
engine_glut_750_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
...
void
engine_glut_750_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
...
void
engine_glut_750_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_750_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_750_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_750_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_750_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_750_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_750_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_750_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_750_timer,
                 v);
}

void
engine_glut_750_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_750_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_750_DEFAULT_HEIGHT));
...
void
engine_glut_750_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_750_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_750_idle);
  else
    glutIdleFunc (NULL);
}
