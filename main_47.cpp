         ...
         enum Engine_ModeType mode_in)
{
  ...
  switch (mode_in)
  {
    ...
    case ENGINE_MODE_989:
    {
      struct Engine_OpenGL_GLUT_989_CBData cb_data_s;

      cb_data_s.scaleFactor = ENGINE_GLUT_989_DEFAULT_SCALE_FACTOR;
      cb_data_s.columns = ENGINE_GLUT_989_DEFAULT_WIDTH / cb_data_s.scaleFactor;
      cb_data_s.rows = ENGINE_GLUT_989_DEFAULT_HEIGHT / cb_data_s.scaleFactor;
      ...
      cb_data_s.mouseX = ENGINE_GLUT_989_DEFAULT_WIDTH / 2;
      cb_data_s.mouseY = ENGINE_GLUT_989_DEFAULT_HEIGHT / 2;
      cb_data_s.mouseLMBPressed = false;
      cb_data_s.mouseLMBClickedX = 0;
      cb_data_s.mouseLMBClickedY = 0;

      // initialize GLUT
      glutInit (&argc_in, argv_in);
      glutSetOption (GLUT_ACTION_ON_WINDOW_CLOSE, GLUT_ACTION_GLUTMAINLOOP_RETURNS);
      glutInitDisplayMode (GLUT_RGBA | GLUT_DOUBLE | GLUT_ALPHA | GLUT_DEPTH);
      glutInitWindowSize (ENGINE_GLUT_989_DEFAULT_WIDTH, ENGINE_GLUT_989_DEFAULT_HEIGHT);

      int window_i = glutCreateWindow ("engine GLUT 989");
      glutSetWindow (window_i);
      glutSetWindowData (&cb_data_s);
      ...
      glutDisplayFunc (engine_glut_989_draw);
      glutReshapeFunc (engine_glut_989_reshape);
      glutVisibilityFunc (engine_glut_989_visible);

      glutKeyboardFunc (engine_glut_989_key);
      glutKeyboardUpFunc (engine_glut_989_key_up);
      glutSpecialFunc (engine_glut_989_key_special);
      glutMouseFunc (engine_glut_989_mouse_button);
      glutMotionFunc (engine_glut_989_mouse_move);
      glutPassiveMotionFunc (engine_glut_989_mouse_move);

      glutTimerFunc (100, engine_glut_989_timer, 0);

      glutCreateMenu (engine_glut_989_menu);
      glutAddMenuEntry (ACE_TEXT_ALWAYS_CHAR ("wireframe"), 0);
      glutAttachMenu (GLUT_RIGHT_BUTTON);

      if (!cb_data_s.shader.loadFromFile (ACE_TEXT_ALWAYS_CHAR ("glut_989.vert"),
                                          ACE_TEXT_ALWAYS_CHAR ("glut_989.frag")))
      {
        ACE_DEBUG ((LM_ERROR,
                    ACE_TEXT ("failed to load shader, aborting\n")));
        break;
