...
void
engine_glut_345_reshape (int width_in, int height_in)
{
...
void
engine_glut_345_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_345_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_345_CBData*> (glutGetWindowData ());
...
void
engine_glut_345_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_345_CBData* cb_data_p =
...
void
engine_glut_345_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_345_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_345_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_345_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_345_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_345_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_345_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_345_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_345_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_345_timer (int v)
{
  ...
  //  glutPostRedisplay();
  //} // end IF
  glutTimerFunc (1000 / 30, engine_glut_345_timer, v);
}

void
engine_glut_345_draw (void)
{
  struct Engine_OpenGL_GLUT_345_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_345_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  //;[W,0].map(l=>pointLight(100,180,255,0,l,W));
  static GLfloat light_position_a[] = {0.0f, static_cast<float> (ENGINE_GLUT_345_DEFAULT_WIDTH), static_cast<float> (ENGINE_GLUT_345_DEFAULT_WIDTH)};
  static GLfloat light_position_2[] = {0.0f, 0.0f, static_cast<float> (ENGINE_GLUT_345_DEFAULT_WIDTH)};
  glLightfv (GL_LIGHT0, GL_POSITION, light_position_a);
  glLightfv (GL_LIGHT1, GL_POSITION, light_position_2);
  static GLint light_ambient_a[] = {100, 180, 255, 255};
  static GLint light_diffuse_a[] = {100, 180, 255, 255};
  ...
  static GLfloat material_shininess_f = 5.0f;
  ...
  glRotatef ((180.0f / static_cast<float> (M_PI)) * cb_data_p->f, 0.0f, 1.0f, 0.0f);
  ...
  while (i > 0.0f)
  {
    ...
    i -= static_cast<float> (M_PI) / 512.0f;
    glTranslatef (199.0f, 0.0f, 0.0f);
    glRotatef ((180.0f / static_cast<float> (M_PI)) * (i * 1.5f + cb_data_p->f * 3.0f), 0.0f, 1.0f, 0.0f);
...
void
engine_glut_345_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_345_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_345_idle);
  else
    glutIdleFunc (NULL);
}
