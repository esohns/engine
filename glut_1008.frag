...
void
cameraSetup (out vec3 prp, out vec3 scp, vec2 fragCoord)
{
  float time=iTime+timeadd;
...
void
raymarching (out vec4 fragColor, vec3 prp, vec3 scp)
{
  ...
  if (f<maxd)
  {
    ...
    if (s.y==2.0)
    {
      ...
      if (f<maxd)
      {
        ...
        fragColor=vec4(objColor*0.8*(1.0-f0*.03),1.0);
    }
    else
      fragColor=vec4(objColor*(1.0-f0*.03),1.0);
  }
  else
...
main ()
{
  ...
  vec3 prp;
  vec3 scp;
  cameraSetup(prp,scp,gl_FragCoord.xy);
  raymarching(o,prp,scp);
