...
vec4 formula(vec4 p) {
    p.xz = abs(p.xz + 1.)-abs(p.xz - 1.2 + 0.0) - p.xz;
    p.y-=.25;
    p.xy *= rot(radians(35.));
        p=p*2./clamp(dot(p.xyz,p.xyz),.2,min(1.05, 0.95 + 0.0));
  return p;
}
...
float de(vec3 pos) {
#ifdef WAVES
  pos.y+=sin(pos.z-t*6.+0.0)*.15; //waves!
#endif
  float hid=0.;
  vec3 tpos=pos;
...

{
  ...
  dir.y-=.02;
  float sunsize= 7. - 0.0; // responsive sun size
  float an=atan(dir.x,dir.y)+iTime * 1.5 +0.0; // angle for drawing and rotating sun
  float s=pow(clamp(1.0-length(dir.xy)*sunsize-abs(.2-mod(an,.4)),0.,1.),.1); // sun
  float sb=pow(clamp(1.0-length(dir.xy)*(sunsize-.2)-abs(.2-mod(an,.4)),0.,1.),.1); // sun border
  float sg=pow(clamp(1.0-length(dir.xy)*(sunsize - 4.5 - 0.0 )-.5*abs(.2-mod(an,.4)),0.,1.),3.); // sun rays
