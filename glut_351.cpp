...
void
engine_glut_351_reshape (int width_in, int height_in)
{
...
void
engine_glut_351_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_351_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_351_CBData*> (glutGetWindowData ());
...
void
engine_glut_351_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_351_CBData* cb_data_p =
...
void
engine_glut_351_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_351_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_351_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_351_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_351_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_351_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_351_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_351_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_351_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_351_timer (int v)
{
  ...
  //  glutPostRedisplay();
  //} // end IF
  glutTimerFunc (1000 / 30, engine_glut_351_timer, v);
}

void
engine_glut_351_draw (void)
{
  struct Engine_OpenGL_GLUT_351_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_351_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_351_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_351_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_351_idle);
  else
    glutIdleFunc (NULL);
}
