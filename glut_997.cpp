...
void
engine_glut_997_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_997_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_997_CBData*> (glutGetWindowData ());
...
void
engine_glut_997_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
...
void
engine_glut_997_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
...
void
engine_glut_997_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
...
void
engine_glut_997_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_997_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_997_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_997_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_997_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_997_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_997_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_997_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_997_timer,
                 v);
}

void
engine_glut_997_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_997_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_997_DEFAULT_HEIGHT));
...
void
engine_glut_997_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_997_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_997_idle);
  else
    glutIdleFunc (NULL);
}
