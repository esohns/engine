...
Main
  ...
  if (i < k)
  {
    ...
    if (gl_FragColor.x>R.x)gl_FragColor.y=.5*R.y,gl_FragColor.z=-10.;
    if (gl_FragColor.x<0.)gl_FragColor.y=.5*R.y,gl_FragColor.z=10.;
  }

  if (I == 1/* || keyClick(32)*/)
  {
    if (i > k)
      gl_FragColor = vec4(R+i,0,0); 
    else
      gl_FragColor = vec4(.5*R + .25*R.y* cos( 6.28*i/k + vec2(0,1.57)), 0,0 );
  }
}
