...
void
engine_glut_944_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_944_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_944_CBData*> (glutGetWindowData ());
...
void
engine_glut_944_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
...
void
engine_glut_944_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
...
void
engine_glut_944_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
...
void
engine_glut_944_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_944_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_944_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_944_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_944_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_944_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_944_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_944_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_944_timer,
                 v);
}

void
engine_glut_944_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_944_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_944_DEFAULT_HEIGHT));
...
void
engine_glut_944_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_944_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_944_idle);
  else
    glutIdleFunc (NULL);
}
