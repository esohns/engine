...
void
engine_glut_989_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_989_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_989_CBData*> (glutGetWindowData ());
...
void
engine_glut_989_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
...
void
engine_glut_989_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
...
void
engine_glut_989_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
...
void
engine_glut_989_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_989_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_989_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_989_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
    {
      ...
      {
        cb_data_p->mouseLMBClickedX = x;
        cb_data_p->mouseLMBClickedY = y;
      } // end IF
      else
      {
        cb_data_p->mouseLMBClickedX = 0;
        cb_data_p->mouseLMBClickedY = 0;
      }
      break;
    }
    default:
...
void
engine_glut_989_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_989_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_989_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_989_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_989_timer,
                 v);
}

void
engine_glut_989_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_989_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_989_DEFAULT_HEIGHT));
  ...
  glProgramUniform4f (cb_data_p->shader.id_, cb_data_p->mouseLoc,
                      ...
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_989_DEFAULT_HEIGHT - 1, ENGINE_GLUT_989_DEFAULT_HEIGHT - 1, 0)),
                      static_cast<GLfloat> (cb_data_p->mouseLMBPressed ? cb_data_p->mouseLMBClickedX : 0),
                      static_cast<GLfloat> (cb_data_p->mouseLMBPressed ? cb_data_p->mouseLMBClickedY : 0));
...
void
engine_glut_989_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_989_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_989_idle);
  else
    glutIdleFunc (NULL);
}
