...
vec3
greeble (vec3 p, float findex, float phase)
{
...
f (vec3 p)
{
  ...
  float hash = noise2_(vec2(h.xz));
  h = ivec3(p+42.0);
  float phase = noise2_(vec2(h.xz));
  vec3 q = p;
  q.xz = mod(q.xz, 1.0);
  q -= 0.5;
