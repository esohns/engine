...
float
handleCorner (vec2 p, vec2 a, vec2 b, vec2 c)
{
...
float
bokehtria (vec2 p, vec2 a, vec2 b, vec2 c)
{
...
float
bokehsquare (vec2 p, vec2 a, vec2 b, vec2 c, vec2 d, float scale)
{
...
main ()
{
  ...
  vec2 p = (gl_FragCoord.xy - iResolution*.5) / iResolution.x;
