...
void
engine_glut_277_reshape (int width_in, int height_in)
{
...
void
engine_glut_277_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_277_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_277_CBData*> (glutGetWindowData ());
...
void
engine_glut_277_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_277_CBData* cb_data_p =
...
void
engine_glut_277_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_277_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_277_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_277_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_277_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_277_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_277_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_277_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_277_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_277_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_277_timer,
                 v);
}

void
engine_glut_277_draw (void)
{
  ...
  glProgramUniform2f (cb_data_p->programId, cb_data_p->resolutionLoc,
                      static_cast<float> (ENGINE_GLUT_277_DEFAULT_WIDTH),
                      static_cast<float> (ENGINE_GLUT_277_DEFAULT_HEIGHT));
  ...
  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_277_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_277_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_277_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_277_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_277_idle);
  else
    glutIdleFunc (NULL);
}
