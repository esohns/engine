...
void
engine_glut_313_reshape (int width_in, int height_in)
{
...
void
engine_glut_313_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_313_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_313_CBData*> (glutGetWindowData ());
...
void
engine_glut_313_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_313_CBData* cb_data_p =
...
void
engine_glut_313_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_313_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_313_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_313_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_313_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_313_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_313_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_313_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_313_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_313_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_313_timer,
                 v);
}

void
engine_glut_313_draw (void)
{
  struct Engine_OpenGL_GLUT_313_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_313_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glProgramUniform2f (cb_data_p->programId, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_312_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_312_DEFAULT_HEIGHT));
...
#error missing implementation, aborting
  ...
  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_313_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_313_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_313_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_313_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_313_idle);
  else
    glutIdleFunc (NULL);
}
