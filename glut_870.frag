...
geometry
enchancedRayMarcher (vec3 o, vec3 d, int maxI)
{
  geometry mp;
...
geometry
vanillaRayMarch (vec3 o, vec3 d, int maxI)
{
  geometry mp;
...
getNormalHex (vec3 pos)
{
  ...
  return normalize(vec3(scene(pos+vec3(EPSILON,0,0)).dist-d,
                        scene(pos+vec3(0,EPSILON,0)).dist-d,
                        scene(pos+vec3(0,0,EPSILON)).dist-d));
}

float
getAO (vec3 hitp, vec3 normal, float dist)
{
...
vec3
Sky (vec3 rd, bool showSun, vec3 lightDir)
{
  float sunSize = 1.;
...
vec3
doColor (vec3 sp, vec3 rd, vec3 sn, vec3 lp, geometry obj)
{
