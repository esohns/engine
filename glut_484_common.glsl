...
#define light_dir normalize (vec3 (0.741, 1.0, 0.58))
...
float Pressure (float rho) { return (rho / rest_density - 1.0) / max (rho * rho, 0.001); }
...
vec3
KernelPoly6Grad (vec3 dx, float d)
{
  float c = KERNEL_POLY6_NORMALIZATION / cub (cub (d));
  float r = length (dx);
...
vec4 packParticles(Particle p0, Particle p1, vec3 pos)
{
  ...
  uvec4 data = uvec4 (packMassPos(p0.mass, p0.pos),
