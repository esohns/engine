...
float
smpl (vec3 p, vec3 dir, float dist)
{
  return smoothstep(-dist,dist,scene(p+dir*dist));
}

vec3
pixel_color (vec2 uv, float time)
{
...
main ()
{
  ...
  gl_FragColor = vec4(0);
  //float b = bayer(ivec2(gl_FragCoord.xy));
  for (int i = 0; i < AA_SAMPLES+int(min(0,iFrame)); i++)
  {
    ...
    float blur = 0.;
#endif
    gl_FragColor += vec4(pixel_color(uv2, iTime+blur), 1.);
  }
  gl_FragColor.xyz = sqrt(gl_FragColor.xyz/gl_FragColor.w);

  gl_FragColor.xyz = smin(gl_FragColor.xyz,vec3(1),0.1);
}
