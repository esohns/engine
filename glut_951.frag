...
float
rayPlane (vec3 ro, vec3 rd, vec4 p)
{
...
float
rayMarch (vec3 ro, vec3 rd, float initt)
{
  float t = initt;
...
float
softShadow (vec3 ps, vec3 ld, float mint, float k)
{
  float res = 1.0;
  float t = mint*3.0;
...
effect (vec2 p)
{
  const vec3 ro = .66*vec3(5.0, -1., 2.3);
