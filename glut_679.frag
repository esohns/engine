...
fbm2 (vec2 p)
{
  return 0.5 * noise2_(p) + 0.25 * noise2_(2. * rot(2.) * (p + 1.)) + 0.125 * noise2_(4. * rot(4.) * (p + 2.)) + 0.0625 * noise2_(8. * rot(6.) * (p + 3.)) + 0.0625 * noise2_(16. * rot(8.) * (p + 4.));
}

float
fbm3 (vec3 p)
{
  return 0.5 * noise3_(p) + 0.25 * noise3_(2. * (p + 1.)) + 0.125 * noise3_(4. * (p + 2.)) + 0.0625 * noise3_(8. * (p + 3.)) + 0.0625 * noise3_(16. * (p + 4.));
}

float
distEllipsoid (vec3 p, vec3 r)
{
...
float
distScene (vec3 pos, out float colorVariation)
{
  ...
  float hair = -100. * (length(p) - 0.16) - noise2_(50. * vec2(angle + 0.01 * sin(50. * f), 0.25 * f));
  ...
  distHair = smoothmax(distHair, length(p - vec3(0., 0., -0.06)) - 0.055, 0.02) - 0.02 * noise3_(50. * p);
  ...
  p.y /= 3.;
  float eyebrow = -100. * (distEyebrow - 0.004) + 0.5 * noise2_(400. * p.xy);
  ...
  colorVariation += smoothstep(0.4, 0.6, -60. * (distEyelashes - 0.01) + 0.15 * noise2_(600. * p.xy));
...
float
getShadow (vec3 pos, vec3 light, vec3 normal)
{
...
render (vec2 uv)
{
  ...
  if(hit)
    col = mix(col, ink, colorVariation);
