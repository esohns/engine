...
CalcPixelRandom ()
{
  vec4 s1 = sin(iTime * 3.3422 + gl_FragCoord.xxxx * vec4(324.324234, 563.324234, 657.324234, 764.324234)) * 543.3423;
  vec4 s2 = sin(iTime * 1.3422 + gl_FragCoord.yyyy * vec4(567.324234, 435.324234, 432.324234, 657.324234)) * 654.5423;
...
vec3
DomainRepeatXZGetTile (vec3 vPos, vec2 vRepeat, out vec2 vTile)
{
  vec3 vResult = vPos;
...
vec3
DomainRepeatXZ (vec3 vPos, vec2 vRepeat)
{
  vec3 vResult = vPos;
  vec2 vTilePos = (vPos.xz / vRepeat) + 0.5;
...
vec3
DomainRepeatY (vec3 vPos, float fSize)
{
  vec3 vResult = vPos;
...
vec3
DomainRotateSymmetry (vec3 vPos, float fSteps)
{
...
float
GetDistanceXYTorus (vec3 p, float r1, float r2)
{
...
float
GetDistanceYZTorus (vec3 p, float r1, float r2)
{
...
float
GetDistanceCylinderY (vec3 vPos, float r)
{
  return length(vPos.xz) - r;
}

float
GetDistanceBox (vec3 vPos, vec3 vSize)
{
...
float
GetDistanceRoundedBox (vec3 vPos, vec3 vSize, float fRadius)
{
...
C_Material
GetObjectMaterial (vec3 vObjId, vec3 vPos, vec3 vNormal)
{
  C_Material mat;
...
void
ApplyAtmosphere (inout vec3 col, C_Ray ray, C_HitInfo intersection)
{
#ifdef ENABLE_FOG
...
void
Raymarch (C_Ray ray, out C_HitInfo result, float fMaxDist, int maxIter)
{
...
float
GetShadow (vec3 vPos, vec3 vLightDir, float fLightDistance)
{
  C_Ray shadowRay;
...
float
Schlick (vec3 vNormal, vec3 vView, float fR0, float fSmoothFactor)
{
...
float
GetDiffuseIntensity (vec3 vLightDir, vec3 vNormal)
{
  return max(0.0, dot(vLightDir, vNormal));
}

float
GetBlinnPhongIntensity (C_Ray ray, C_Material mat, vec3 vLightDir, vec3 vNormal)
{
...
float
GetAmbientOcclusion (C_Ray ray, C_HitInfo intersection, vec3 vNormal)
{
...
vec3
GetObjectLighting (C_Ray ray, C_HitInfo intersection, C_Material material, vec3 vNormal, vec3 cReflection)
{
  vec3 cScene;
...
vec3
GetSceneColourSimple (C_Ray ray)
{
...
#endif
  vec2 vUV = ( vPixelCoord / iResolution );
...
main ()
{
  ...
  vec2 vMouse = iMouse.xy / iResolution;
  if(iMouse.x <= 0.0)
  ...
  GetCameraRayLookat( vCameraPos, vec3(0.0, -0.5, 0.2), gl_FragCoord.xy, ray);
  ...
  vec2 vUV = ((gl_FragCoord.xy / iResolution) - 0.5) * 2.0;
