...
float
planeSDF (vec3 point_)
{
  return point_.y;
}

float
boxSDF (vec3 point_, vec3 bounds)
{
    vec3 q = abs(point_) - bounds;
    return length(max(q, 0.0)) + min(max(q.x, max(q.y, q.z)), 0.0);
}

vec4
repeatBoxSDF (vec3 point_)
{
  vec3 rootPoint = floor(vec3(point_.x / LINE_SPACE, point_.y / LINE_SPACE, point_.z / LINE_LENGTH)); 
  ...
  for (float x = -1.0; x <= 1.1; x++)
    for (float y = -1.0; y <= 1.1; y++)
      for (float z = -1.0; z <= 1.1; z++)
      {
        ...
        float sdf = boxSDF(point_ - boxCenter, boxSize);
        if (sdf < minSDF)
        {
          mainColor = color;
          minSDF = sdf;
...
float
cylinderSDF (vec3 point_, float radius)
{
  return length(point_.xy) - radius;
}

float
multiplyObjects (float o1, float o2)
{
  return max(o1, o2);   
}

vec3
spaceBounding (vec3 point_)
{
  return vec3(sin(point_.z * 0.15) * 5.0, cos(point_.z * 0.131) * 5.0, 0.0); 
}

vec4
objectSDF (vec3 point_)
{
  point_ += spaceBounding(point_);
    
  vec4 lines = repeatBoxSDF(point_);
  float cylinder = cylinderSDF(point_, BOUNDING_CYLINDER);
  float insideCylinder = -cylinderSDF(point_, INSIDE_CYLINDER);
...
vec3
rayMarch (vec3 rayOrigin, vec3 rayDirection, out vec3 color)
{
  color = vec3(0.0);
  float dist = 0.0;
  for (float i = 0.0; i < RAYMARCH_ITERATIONS; i++)
  {
    ...
    dist += sdfData.a;
    if (length(rayOrigin.xy) > BOUNDING_CYLINDER + 10.0)
      break;
  }
