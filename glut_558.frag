...
vec4
pselect (vec4 a, vec4 b)
{
  return a.w < b.w ? a : b;
}

float
torus (vec3 a, vec3 b, vec3 pos)
{
  pos -= 0.5*(a+b);
...
mat4
permute (vec3 e, vec3 f, vec3 g, vec3 h, float p)
{
...
vec2
castRay (vec3 ro, vec3 rd, float maxd)
{
  float precis = 0.0001;
  float h=precis*2.0;
  float t = 0.0;
