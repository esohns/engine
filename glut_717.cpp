...
void
engine_glut_717_reshape (int width_in, int height_in)
{
...
void
engine_glut_717_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_717_CBData*> (glutGetWindowData ());
...
void
engine_glut_717_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
...
void
engine_glut_717_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
...
void
engine_glut_717_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
...
void
engine_glut_717_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_717_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_717_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_717_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_717_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_717_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_717_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_717_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_717_timer,
                 v);
}

void
engine_glut_717_draw (void)
{
...
//  last_render_time = tp2;
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_717_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_717_DEFAULT_HEIGHT));
...
void
engine_glut_717_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_717_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_717_idle);
  else
    glutIdleFunc (NULL);
}
