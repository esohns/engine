...
void
engine_glut_627_reshape (int width_in, int height_in)
{
...
void
engine_glut_627_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_627_CBData*> (glutGetWindowData ());
...
void
engine_glut_627_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
...
void
engine_glut_627_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
...
void
engine_glut_627_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
...
void
engine_glut_627_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_627_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_627_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_627_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_627_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_627_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_627_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_627_timer,
                 v);
}

void
engine_glut_627_draw (void)
{
  struct Engine_OpenGL_GLUT_627_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_627_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_627_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_627_DEFAULT_HEIGHT));
...
#error missing implementation, aborting
  ...
  static float frame_rate_f = 60.0f;
  if (elapsed_seconds.count () > 1.0f)
  {
    frame_rate_f = last_frame_count_i / elapsed_seconds.count ();
  ...
  glProgramUniform1f (cb_data_p->shader.id_, cb_data_p->frameRateLoc,
                      static_cast<GLfloat> (frame_rate_f));

  glProgramUniform4f (cb_data_p->shader.id_, cb_data_p->mouseLoc,
                      static_cast<GLfloat> (cb_data_p->mouseX),
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_627_DEFAULT_HEIGHT - 1, ENGINE_GLUT_627_DEFAULT_HEIGHT - 1, 0)),
...
void
engine_glut_627_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_627_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_627_idle);
  else
    glutIdleFunc (NULL);
}
