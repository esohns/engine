...
void
engine_glut_911_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_911_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_911_CBData*> (glutGetWindowData ());
...
void
engine_glut_911_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
...
void
engine_glut_911_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
...
void
engine_glut_911_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
...
void
engine_glut_911_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_911_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_911_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_911_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_911_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_911_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_911_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_911_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_911_timer,
                 v);
}

void
engine_glut_911_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_911_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_911_DEFAULT_HEIGHT));

  glProgramUniform4f (cb_data_p->shader1.id_, cb_data_p->S1mouseLoc,
                      static_cast<GLfloat> (cb_data_p->mouseX),
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_911_DEFAULT_HEIGHT - 1, ENGINE_GLUT_911_DEFAULT_HEIGHT - 1, 0)),
...
void
engine_glut_911_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_911_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_911_idle);
  else
    glutIdleFunc (NULL);
}
