...
float
sdCappedTorus (vec3 p, vec2 sc, float ra, float rb)
{
  p.x = abs (p.x);
...
float
sdOctogonPrism (vec3 p, float r, float h)
{
...
float
sdCappedCone (vec3 p, float h, float r1, float r2)
{
  vec2 q = vec2 (length (p.xz), p.y);
...
float
sdSolidAngle (vec3 pos, vec2 c, float ra)
{
...
float
sdRhombus (vec3 p, float la, float lb, float h, float ra)
{
  p = abs (p);
  vec2 b = vec2 (la, lb);
...
float
sdHorseshoe (vec3 p, vec2 c, float r, float le, vec2 w)
{
  p.x = abs (p.x);
  float l = length (p.xy);
...
float
sdU (vec3 p, float r, float le, vec2 w)
{
...
map (vec3 pos)
{
  ...
  if (sdBox (pos - vec3 (0.0, 0.3, -1.0), vec3 (0.35, 0.3, 2.5)) < res.x)
  {
    ...
    res = opU (res, vec2 (sdSolidAngle (pos - vec3 (0.0, 0.0, -3.0), vec2 (3.0, 4.0) / 5.0, 0.4), 49.13));
  }
  ...
  if (sdBox (pos - vec3 (1.0, 0.3, -1.0), vec3 (0.35, 0.3, 2.5)) < res.x)
  {
    ...
    res = opU (res, vec2 (sdCapsule (pos - vec3 (1.0, 0.0, -1.0), vec3 (-0.1, 0.1, -0.1), vec3 (0.2, 0.4, 0.2), 0.1), 31.9));
...
float
checkersGradBox (vec2 p, vec2 dpdx, vec2 dpdy)
{
...
vec3
render (vec3 ro, vec3 rd, vec3 rdx, vec3 rdy)
{ 
  ...
  if (m > -0.5)
  {
...

    {
      ...
      lin += col * 0.60 * dif * vec3 (0.4, 0.6, 1.15);
