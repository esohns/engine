...
void
engine_glut_853_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_853_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_853_CBData*> (glutGetWindowData ());
  ...
  switch (key_in)
  {
    case 27: /* Escape */
      ...
      break;
    case 32: /* Space */
      cb_data_p->isSpaceKeyPressed = !cb_data_p->isSpaceKeyPressed;
      break;
    case 'p':
...
void
engine_glut_853_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
...
void
engine_glut_853_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
  ...
  {
    case 101: // Up
      cb_data_p->isUpKeyPressed = !cb_data_p->isUpKeyPressed;
      break;
    default:
      break;
  } // end SWITCH
...
void
engine_glut_853_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
  ...
  {
    //case 101: // Up
    //  cb_data_p->isUpKeyPressed = false;
    //  break;
    default:
      break;
  } // end SWITCH
...
void
engine_glut_853_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_853_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_853_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_853_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_853_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_853_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_853_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_853_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_853_timer,
                 v);
}

void
engine_glut_853_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_853_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_853_DEFAULT_HEIGHT));

  glProgramUniform4i (cb_data_p->shader1.id_, cb_data_p->S1keysLoc,
                      static_cast<GLint> (cb_data_p->isSpaceKeyPressed ? 1 : 0),
                      static_cast<GLint> (cb_data_p->isUpKeyPressed ? 1 : 0),
  ...
  glProgramUniform1f (cb_data_p->shader5.id_, cb_data_p->S5timeLoc,
                      static_cast<GLfloat> (d.count () * 0.001f));

  glProgramUniform4f (cb_data_p->shader5.id_, cb_data_p->S5mouseLoc,
                      static_cast<GLfloat> (cb_data_p->mouseX),
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_853_DEFAULT_HEIGHT - 1, ENGINE_GLUT_853_DEFAULT_HEIGHT - 1, 0)),
...
void
engine_glut_853_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_853_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_853_idle);
  else
    glutIdleFunc (NULL);
}
