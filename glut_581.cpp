...
void
engine_glut_581_reshape (int width_in, int height_in)
{
...
void
engine_glut_581_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_581_CBData*> (glutGetWindowData ());
...
void
engine_glut_581_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
...
void
engine_glut_581_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
...
void
engine_glut_581_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
...
void
engine_glut_581_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_581_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_581_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_581_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_581_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_581_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_581_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_581_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_581_timer,
                 v);
}

void
engine_glut_581_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_581_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_581_DEFAULT_HEIGHT));
  ...
  glProgramUniform4f (cb_data_p->shader2.id_, cb_data_p->S2dateLoc,
                      static_cast<GLfloat> (cb_data_p->today.year () - 1900),
                      static_cast<GLfloat> (cb_data_p->today.month () - 1),
                      static_cast<GLfloat> (cb_data_p->today.day ()),
                      static_cast<GLfloat> (cb_data_p->today.hour () * 60 * 60 + cb_data_p->today.minute () * 60 + cb_data_p->today.second ()));

  glProgramUniform4f (cb_data_p->shader2.id_, cb_data_p->S2mouseLoc,
                      static_cast<GLfloat> (cb_data_p->mouseX),
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_581_DEFAULT_HEIGHT - 1, ENGINE_GLUT_581_DEFAULT_HEIGHT - 1, 0)),
...
void
engine_glut_581_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_581_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_581_idle);
  else
    glutIdleFunc (NULL);
}
