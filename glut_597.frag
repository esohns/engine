...
void
CameraSetup (vec2 uv, vec3 position, vec3 lookAt, float zoom)
{
  cam.p = position;
  cam.lookAt = lookAt;
...
float
DistLine (vec3 ro, vec3 rd, vec3 p)
{
  return length(cross(p-ro, rd));
}

vec2
within (vec2 uv, vec4 rect)
{
...
vec3
opCheapBend (vec3 p, float strength)
{
...
float
CB_Dist (rc q, vec3 rd)
{
  vec3 rC = ((2.*step(0., rd)-1.)*q.h-q.p)/rd;
  return min(min(rC.x, rC.y), rC.z)+.01;
}
...
vec3
pModPolar (inout vec3 p, float repetitions, float fix)
{
...
Background (ray r)
{
  ...
  for(float i=0.; i<BG_STEPS; i++)
  {
    ...
    q.xz = mod(p.xz, size)-h;

    vec3 cP = vec3(0, N21(id)*4.-2., 0);

    float dRayFlame = DistLine(q, r.d, cP);

    float dOriFlame = d + length(cP-p);
...
vec4
render (vec2 uv, ray camRay, float n)
{
  vec3 col = vec3(0.);
...
vec4
Flame (vec2 uv, vec2 p)
{
  uv.x-=.5;
  p.x -= .5;

  vec3 col = vec3(.99, .6, .35);
