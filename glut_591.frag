...
vec2
wavedx (vec2 position, vec2 direction, float speed, float frequency, float timeshift)
{
...
float
getwaves (vec2 position, int iterations)
{
  float iter = 0.0;
  float phase = 6.0;
  float speed = 2.0;
...
float
raymarchwater (vec3 camera, vec3 start, vec3 end, float depth)
{
  vec3 pos = start;
  float h = 0.0;
  float hupper = depth;
...
vec3
normal (vec2 pos, float e, float depth)
{
  vec2 ex = vec2(e, 0);
...
float
intersectPlane (vec3 origin, vec3 direction, vec3 point, vec3 normal)
{ 
  return clamp(dot(point - origin, normal) / dot(direction, normal), -1.0, 9991999.0); 
}

vec3
extra_cheap_atmosphere (vec3 raydir, vec3 sundir)
{
