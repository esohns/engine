...
float
opU (float d1, float d2)
{
  return min(d1,d2);
}

float
opS (float d1, float d2)
{
  return max(-d1, d2);
}
...
float
sdPlane (vec3 p, vec3 p0, vec3 p1, vec3 p2)
{
  return dot(p - p0, normalize(cross(p0 - p1, p0 - p2)));
}

float
sdVQuad (vec3 p, float h0, float h1, float h2, float h3)
{
  float s = QUAD_SIZE;
...
vec3
Shade (MarchResult hit, vec3 direction, vec3 camera)
{
  vec3 color = vec3(1.0);
...
main ()
{
  ...
  {    
    angles = vec3((iMouse.xy / iResolution) * pi, 0);
