...
void
engine_glut_745_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_745_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_745_CBData*> (glutGetWindowData ());
...
void
engine_glut_745_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
...
void
engine_glut_745_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
...
void
engine_glut_745_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
...
void
engine_glut_745_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_745_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_745_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_745_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_745_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_745_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_745_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_745_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_745_timer,
                 v);
}

void
engine_glut_745_draw (void)
{
...
//    std::chrono::duration_cast<std::chrono::milliseconds> (elapsed_time);
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_745_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_745_DEFAULT_HEIGHT));
...
void
engine_glut_745_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_745_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_745_idle);
  else
    glutIdleFunc (NULL);
}
