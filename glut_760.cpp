...
void
engine_glut_760_reshape (int width_in,
                         int height_in)
{
...
void
engine_glut_760_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_760_CBData*> (glutGetWindowData ());
...
void
engine_glut_760_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
...
void
engine_glut_760_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
...
void
engine_glut_760_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
...
void
engine_glut_760_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_760_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_760_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_760_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_760_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_760_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_760_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_760_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_760_timer,
                 v);
}

void
engine_glut_760_draw (void)
{
...
//    std::chrono::duration_cast<std::chrono::milliseconds> (elapsed_time);
  cb_data_p->today = COMMON_DATE_NOW;
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_760_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_760_DEFAULT_HEIGHT));
  ...
  glProgramUniform4f (cb_data_p->shader1.id_, cb_data_p->S1dateLoc,
                      ...
                      static_cast<GLfloat> (cb_data_p->today.hour () * 60 * 60 + cb_data_p->today.minute () * 60 + cb_data_p->today.second ()) + (static_cast<GLfloat> (cb_data_p->today.microsec ()) * 0.000001f));
...
void
engine_glut_760_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_760_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_760_idle);
  else
    glutIdleFunc (NULL);
}
