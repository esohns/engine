...
void
engine_glut_498_reshape (int width_in, int height_in)
{
...
void
engine_glut_498_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_498_CBData*> (glutGetWindowData ());
...
void
engine_glut_498_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
...
void
engine_glut_498_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
...
void
engine_glut_498_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
...
void
engine_glut_498_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_498_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_498_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_498_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_498_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_498_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_498_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_498_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_498_timer,
                 v);
}

void
engine_glut_498_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_498_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_498_DEFAULT_HEIGHT));
  ...
  glProgramUniform4f (cb_data_p->shader.id_, cb_data_p->mouseLoc,
                      ...
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_498_DEFAULT_HEIGHT - 1, ENGINE_GLUT_498_DEFAULT_HEIGHT - 1, 0)),
...
void
engine_glut_498_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_498_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_498_idle);
  else
    glutIdleFunc (NULL);
}
