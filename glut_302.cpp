...
void
engine_glut_302_reshape (int width_in, int height_in)
{
...
void
engine_glut_302_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_302_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_302_CBData*> (glutGetWindowData ());
...
void
engine_glut_302_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_302_CBData* cb_data_p =
...
void
engine_glut_302_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_302_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_302_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_302_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_302_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_302_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_302_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_302_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_302_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_302_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_302_timer,
                 v);
}

void
engine_glut_302_draw (void)
{
  ...
  glProgramUniform2f (cb_data_p->programId, cb_data_p->resolutionLoc,
                      static_cast<float> (ENGINE_GLUT_302_DEFAULT_WIDTH),
                      static_cast<float> (ENGINE_GLUT_302_DEFAULT_HEIGHT));

  glProgramUniform1f (cb_data_p->programId, cb_data_p->timeLoc,
                      static_cast<float> (frame_count_i));

  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_302_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_302_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_302_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_302_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_302_idle);
  else
    glutIdleFunc (NULL);
}
