...
void
engine_glut_298_reshape (int width_in, int height_in)
{
...
void
engine_glut_298_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_298_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_298_CBData*> (glutGetWindowData ());
...
void
engine_glut_298_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_298_CBData* cb_data_p =
...
void
engine_glut_298_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_298_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_298_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_298_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_298_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_298_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_298_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_298_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_298_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_298_timer (int v)
{
  ...
  //  glutPostRedisplay();
  //} // end IF
  glutTimerFunc (1000 / 30, engine_glut_298_timer, v);
}

void
engine_glut_298_draw (void)
{
  struct Engine_OpenGL_GLUT_298_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_298_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  static GLfloat light_position_a[] = {0.0f, static_cast<float> (ENGINE_GLUT_298_DEFAULT_W) *  1.0f, 0.0f};
  static GLfloat light_position_2[] = {0.0f, static_cast<float> (ENGINE_GLUT_298_DEFAULT_W) *  1.0f, 0.0f};
  static GLfloat light_position_3[] = {0.0f, static_cast<float> (ENGINE_GLUT_298_DEFAULT_W) * -1.0f, 0.0f};
  ...
  static float P = static_cast<float> (M_PI_2);
  glRotatef ((180.0f / static_cast<float> (M_PI)) * P, 1.0f, 0.0f, 0.0f);
  for (float e = P; e > -P; e -= P / 8.0f)
  {
    float c = ENGINE_GLUT_298_DEFAULT_R * std::cos (e + cb_data_p->i);
    float y = ENGINE_GLUT_298_DEFAULT_R * std::sin (e + cb_data_p->i);

    glPushMatrix ();

    glTranslatef (0.0f, 0.0f, y / 2.0f);
    float Y = y / 200.0f;
    glScalef (Y, Y, 0.1f);

    cb_data_p->wireframe ? glutWireTorus  (20.0, c / 2.0, 32, 32)
                         : glutSolidTorus (20.0, c / 2.0, 32, 32);
  ...
  cb_data_p->i = std::fmod (cb_data_p->i - (P / 256.0f), P / 8.0f);

  glutSwapBuffers ();
}

void
engine_glut_298_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_298_visible (int vis)
{
  glutIdleFunc ((vis == GLUT_VISIBLE) ? engine_glut_298_idle : NULL);
}
