...
main ()
{
  ...
  float height = 0.0;//1. + cos(time);
  float dist = 5.0;
  vec3 ro = vec3 (dist * cos (time), height, dist * sin (time));
  //ro.yz *= Rot(-m.y*3.14+1.);
  //ro.xz *= Rot(-m.x*6.2831);

  vec3 rd = GetRayDir (uv, ro, vec3 (0, height, 0), 1.0);
  vec3 col = vec3 (0.0);
...
//    float c = atan (rf.z, rf.y);
    ...
    float h = mix (h21 (ipos), 0.0, 0.5 + 0.5 * cos (u_time));
    col.r += 0.25 + 0.5 * cos (10.0 * rf.y + u_time - 0.5 * PI);
    col.g += 0.25 + 0.5 * cos (10.0 * rf.y + u_time);
    col.b += 0.25 + 0.5 * cos (10.0 * rf.y + u_time + 0.5 * PI);
    col += thc (1.0, 2.5 * length (p) - u_time);

    vec3 e = vec3 (1.0);
    col = col * pal (2.5 * length (p) - u_time, e, e, e, 0.25 * vec3 (0.0, 0.33, 0.66));
    // col *= n.y;
  }
