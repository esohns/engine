...
noise (vec3 x)
{
  ...
  vec2 rg = textureLod (iChannel0, (uv + 0.5) / 470.0, 0.0).yx;
...
float map (vec3 p, int oct)
{
  ...
  if (oct >= 2)
#endif
  f += 0.25 * noise (q); q = q * 2.23;
#if USE_LOD == 1
  if (oct >= 3)
#endif
  f += 0.125 * noise (q); q = q * 2.41;
#if USE_LOD == 1
  if (oct >= 4)
#endif
  f += 0.0625 * noise (q); q = q * 2.62;
#if USE_LOD == 1
  if (oct>=5)
#endif
  f += 0.03125 * noise (q);
...
vec4
raymarch (vec3 ro, vec3 rd, vec3 bgcol, ivec2 px)
{
  // bounding planes	
  const float yb = -3.0;
  const float yt =  0.6;
...
vec4
render (vec3 ro, vec3 rd, ivec2 px)
{
