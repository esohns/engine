...
norm (vec3 p)
{
  ...
  return normalize(map(p,vec3(0)).x-vec3(map(p,e.xyy).x, map(p,e.yxy).x, map(p,e.yyx).x));
}

void
render (out vec4 fragColor, in vec2 fragCoord)
{
...
main ()
{
  ...
  for (i=0.;i<AA + min (iTime, 0.0); i++)
    for(j=0.;j<AA;j++)
    {
      vec2 C2 = vec2(gl_FragCoord.x+px*i,gl_FragCoord.y+px*j);
      render(cl2,C2);
      cl+=cl2;
      rlg=0.;
      hitonce = false;
