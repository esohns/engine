...
float
fresnel (float n1, float n2, float cos_theta)
{
  ...
  for (int j = 0; j < max_intersections; ++j)
  {
    for (int i = 0; i < maxsteps; ++i)
    {
      ...
      if (f(p + d * (k + ss)) * nf < 0.0)
      {
        ...
        {
          float f = nf > 0.0 ? fresnel(1.0, N, dot(-d, n)) : fresnel(N, 1.0, dot(-d, n));
          if (f > 0.5)
          {
...
main ()
{
  ...
  vec3 eye = -forward * eyedistance;
	...
	vec2 uv = zoom * (gl_FragCoord.xy - iResolution.xy * 0.5) / iResolution.x;
