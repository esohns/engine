...
vec3
calcNormal (vec3 pos, float t, float s)
{
  float precis = 0.001 * t;
...
vec3
render (vec3 ro, vec3 rd, float anim)
{
  vec3 col = vec3 (0.0);
...
/*void
mainVR (out vec4 fragColor, vec2 fragCoord, vec3 fragRayOri, vec3 fragRayDir)
{
  float time = iTime * 0.25 + 0.01 * iMouse.x;
  float anim = 1.1 + 0.5 * smoothstep (-0.3, 0.3, cos (0.1 * iTime));
