...
void
engine_glut_295_reshape (int width_in, int height_in)
{
...
void
engine_glut_295_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_295_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_295_CBData*> (glutGetWindowData ());
...
void
engine_glut_295_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_295_CBData* cb_data_p =
...
void
engine_glut_295_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_295_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_295_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_295_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_295_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_295_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_295_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_295_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_295_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_295_timer (int v)
{
  ...
  //  glutPostRedisplay();
  //} // end IF
  glutTimerFunc (1000 / 30, engine_glut_295_timer, v);
}

void
engine_glut_295_draw (void)
{
  struct Engine_OpenGL_GLUT_295_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_295_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  //[-W, 0] .map(i = > pointLight([W], 0, i, W));
  static GLfloat light_position_a[] = {0.0f, static_cast<float> (-ENGINE_GLUT_295_DEFAULT_W), static_cast<float> (ENGINE_GLUT_295_DEFAULT_W)};
  static GLfloat light_position_2[] = {0.0f, 0.0f, static_cast<float> (ENGINE_GLUT_295_DEFAULT_W)};
  ...
  for (float i = 0.0f; i < 2.0f * static_cast<float> (M_PI); i += static_cast<float> (M_PI) / 256.0f)
  {
    glPushMatrix ();
    glRotatef ((180.0f / static_cast<float> (M_PI)) * (i - cb_data_p->f), 0.0f, 1.0f, 0.0f);

    //glTranslatef (std::sin (std::tan (i)) * 120.0f,
    //              std::tan (std::atan (std::tan (i + cb_data_p->f))) * 50.0f,
    //              std::cos (std::tan (i)) * 120.0f); // V1
    glTranslatef (std::sin (std::tan (i)) * 120.0f,
                  std::sin (std::atan (std::tan (i + cb_data_p->f))) * 70.0f,
                  std::cos (std::tan (i)) * 120.0f); // V2

    cb_data_p->wireframe ? glutWireSphere (std::cos (i + cb_data_p->f) * 5.0f, 16, 16)
                         : glutSolidSphere (std::cos (i + cb_data_p->f) * 5.0f, 16, 16);
    glPopMatrix ();
  } // end FOR
...
void
engine_glut_295_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_295_visible (int vis)
{
  glutIdleFunc ((vis == GLUT_VISIBLE) ? engine_glut_295_idle : NULL);
}
