...
void
engine_glut_663_reshape (int width_in, int height_in)
{
...
void
engine_glut_663_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_663_CBData*> (glutGetWindowData ());
...
void
engine_glut_663_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
...
void
engine_glut_663_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
...
void
engine_glut_663_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
...
void
engine_glut_663_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_663_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_663_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_663_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_663_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_663_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_663_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_663_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_663_timer,
                 v);
}

void
engine_glut_663_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader1.id_, cb_data_p->S1resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_663_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_663_DEFAULT_HEIGHT));
...
void
engine_glut_663_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_663_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_663_idle);
  else
    glutIdleFunc (NULL);
}
