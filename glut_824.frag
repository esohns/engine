...
vec2
complex_mul (vec2 factorA, vec2 factorB)
{
  return vec2( factorA.x*factorB.x - factorA.y*factorB.y, factorA.x*factorB.y + factorA.y*factorB.x);
}

vec2
spiralzoom (vec2 domain, vec2 center, float n, float spiral_factor, float zoom_factor, vec2 pos)
{
...
vec2
complex_div (vec2 numerator, vec2 denominator)
{
...
float
circle (vec2 uv, vec2 aspect, float scale)
{
...
float
smoothcircle (vec2 uv, vec2 aspect, float radius, float ramp)
{
  return 0.5 - sigmoid( ( length( (uv - 0.5) * aspect) - radius) * ramp) * 0.5;
}

float
conetip (vec2 uv, vec2 pos, float size, float min)
{
...
float
warpFilter (vec2 uv, vec2 pos, float size, float ramp)
{
  return 0.5 + sigmoid( conetip(uv, pos, size, -16.) * ramp) * 0.5;
}

vec2
vortex_warp (vec2 uv, vec2 pos, float size, float ramp, vec2 rot)
{
  vec2 aspect = vec2(1.,iResolution.y/iResolution.x);
...
vec2
vortex_pair_warp (vec2 uv, vec2 pos, vec2 vel)
{
  vec2 aspect = vec2(1.,iResolution.y/iResolution.x);
  float ramp = 5.;
...
mouseDelta ()
{
  ...
  if(oldMouse.z > pixelSize.x && oldMouse.z > 0.0 &&
     nowMouse.z > pixelSize.x && nowMouse.z > 0.0)
...
main ()
{
  ...
  vec2 uv_red = uv + vec2(dx.x, dy.x)*pixelSize*8.;

  float new_red = texture(iChannel0, fract(uv_red)).x + (noise.x - 0.5) * 0.0025 - 0.002;
  new_red -= (texture(iChannel1, fract(uv_red + (noise.xy-0.5)*pixelSize)).x -
              texture(iChannel0, fract(uv_red + (noise.xy-0.5)*pixelSize))).x * 0.047;

  if(iFrame<10)
    gl_FragColor = noise; 
  else
    gl_FragColor.x = clamp(new_red, 0., 1.);
}
