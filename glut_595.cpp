...
void
engine_glut_595_reshape (int width_in, int height_in)
{
...
void
engine_glut_595_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_595_CBData*> (glutGetWindowData ());
...
void
engine_glut_595_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
...
void
engine_glut_595_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
...
void
engine_glut_595_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
...
void
engine_glut_595_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_595_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_595_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_595_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
    {
      ...
      if (state == GLUT_DOWN)
      {
        cb_data_p->mouseLMBPositionX = x; cb_data_p->mouseLMBPositionY = y;
      }
      else
      {
        cb_data_p->mouseLMBPositionX = -1; cb_data_p->mouseLMBPositionY = -1;
      }
      break;
    }
...
void
engine_glut_595_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_595_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_595_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_595_timer,
                 v);
}

void
engine_glut_595_draw (void)
{
  struct Engine_OpenGL_GLUT_595_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_595_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
//    std::chrono::duration_cast<std::chrono::milliseconds> (elapsed_time);
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_595_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_595_DEFAULT_HEIGHT));

  glProgramUniform4f (cb_data_p->shader.id_, cb_data_p->mouseLoc,
                      static_cast<GLfloat> (cb_data_p->mouseX),
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_595_DEFAULT_HEIGHT - 1, ENGINE_GLUT_595_DEFAULT_HEIGHT - 1, 0)),
                      static_cast<GLfloat> (cb_data_p->mouseLMBPressed ? cb_data_p->mouseLMBPositionX : -1),
                      static_cast<GLfloat> (cb_data_p->mouseLMBPressed ? cb_data_p->mouseLMBPositionY : -1));
...
void
engine_glut_595_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_595_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_595_idle);
  else
    glutIdleFunc (NULL);
}
