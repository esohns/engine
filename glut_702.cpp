...
void
engine_glut_702_reshape (int width_in, int height_in)
{
...
void
engine_glut_702_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_702_CBData*> (glutGetWindowData ());
...
void
engine_glut_702_key_up (unsigned char key_in,
                        int x,
                        int y)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
...
void
engine_glut_702_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
...
void
engine_glut_702_key_special_up (int key_in,
                                int x,
                                int y)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
...
void
engine_glut_702_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_702_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_702_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_702_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_702_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_702_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_702_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_702_timer (int v)
{
  ...
  glutTimerFunc (1000 / 60,
                 engine_glut_702_timer,
                 v);
}

void
engine_glut_702_draw (void)
{
...
#error missing implementation, aborting
  ...
  glProgramUniform2f (cb_data_p->shader2.id_, cb_data_p->S2resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_702_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_702_DEFAULT_HEIGHT));
...
void
engine_glut_702_idle (void)
{
  //glutPostRedisplay ();
}

void
engine_glut_702_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_702_idle);
  else
    glutIdleFunc (NULL);
}
