...
float
dSegment (vec3 p, vec3 a, vec3 b, float r)
{
  vec3 pa = p - a;
  vec3 ba = b - a;
...
vec3
boxPos (vec3 p, float diff, float o)
{
...
vec3
lighting (vec3 p, vec3 lp, vec3 rd)
{
  vec3 l = lp - p;
...
render (vec3 ro, vec3 rd)
{
  ...
  if (i.y == 0.0)
  {
    ...
    col = .45 + vec3(1) * smoothstep(-.05, .05,abs(fract(guv.x) - .5)*2.) * smoothstep(-.05, .05, abs(fract(guv.y) - .5)*2.);
  }
  if (i.y == 1.0)
    col = vec3(1, .1, 1)*.5;
  if (i.y == 2.0)
  ...
  if(i.y != -1.0)
    col *= lighting(p, lp1, rd)+lighting(p, lp2, rd);
