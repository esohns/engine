...
void
engine_glut_374_reshape (int width_in, int height_in)
{
...
void
engine_glut_374_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_374_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_374_CBData*> (glutGetWindowData ());
...
void
engine_glut_374_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_374_CBData* cb_data_p =
...
void
engine_glut_374_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_374_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_374_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_374_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_374_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_374_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
    {
      if (state == GLUT_DOWN)
      {
        cb_data_p->octaves = (cb_data_p->octaves == 5 ? 1 : cb_data_p->octaves + 1);
      } // end IF
...
void
engine_glut_374_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_374_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_374_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_374_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_374_timer,
                 v);
}

void
engine_glut_374_draw (void)
{
  struct Engine_OpenGL_GLUT_374_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_374_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glProgramUniform2f (cb_data_p->programId, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_374_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_374_DEFAULT_HEIGHT));
...
#error missing implementation, aborting
  ...
  glProgramUniform1i (cb_data_p->programId, cb_data_p->octavesLoc,
                      static_cast<GLint> (cb_data_p->octaves));

  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_374_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_374_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_374_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_374_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_374_idle);
  else
    glutIdleFunc (NULL);
}
