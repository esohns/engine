...
float
rangec (float a, float b, float t)
{
...
float
pMod1 (inout float p, float size)
{
...
vec2
closestPointOnRepeatedLine (vec2 line, vec2 point)
{
...
vec3
closestHelix (vec3 p, float lead, float radius)
{
  p = cartToPolar(p);
  p.y *= radius;
...
void
pModHelix (inout vec3 p, float lead, float radius)
{
...
float
pModHelixScale (inout vec3 p, float lead, float innerRatio)
{
...
float
pModHelixUnwrap (inout vec3 p, float lead, float innerRatio, float t)
{
  float radius = mix(.25, .5, innerRatio);
...
void
addPipe (inout float d, vec3 p, float scale, float tt)
{
...
void
unzipHelixModel (inout float d, inout float scale, inout vec3 p, float lead, float innerRatio, float step, float invert)
{
...
const float OUTLINE = .003;
const float OUTLINE_BOUNDRY = .5;
...
vec3
doLighting (vec3 pos, vec3 nor, vec3 rd)
{
  vec3 col;
...
void
applyFog (inout vec3 color, vec3 pos)
{
...
shadeSurface (vec3 pos, Ray ray)
{
  ...
  vec3 color = doLighting (pos, normal, ray.direction);
...
main ()
{
  ...
  vec3 rd = normalize((vec4(p, -focalLength, 1) * camMat).xyz);

  vec3 color = raymarch(CastRay(camPosition, rd));	

  vec2 uv = gl_FragCoord.xy/iResolution.xy;
  float vig = pow(16. * uv.x * uv.y * (1. - uv.x) * (1. - uv.y), 0.075);
