...
void
CamPolar (out vec3 pos, out vec3 ray, vec3 origin, vec2 rotation, float distance, float zoom, vec2 fragCoord)
{
...
float
WaveCrests (vec3 ipos, in vec2 fragCoord)
{
  vec3 pos = ipos;
  pos *= .2*vec3(1,1,1);
...
float
OceanDistanceField (vec3 pos)
{
  return pos.y - Waves(pos);
}

float
OceanDistanceFieldDetail (vec3 pos)
{
  return pos.y - WavesDetail(pos);
}

vec3
OceanNormal (vec3 pos)
{
  vec3 norm;
...
vec3
ShadeOcean (vec3 pos, vec3 ray, in vec2 fragCoord)
{
