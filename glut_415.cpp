...
void
engine_glut_415_reshape (int width_in, int height_in)
{
...
void
engine_glut_415_key (unsigned char key_in,
                     int x,
                     int y)
{
  struct Engine_OpenGL_GLUT_415_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_415_CBData*> (glutGetWindowData ());
...
void
engine_glut_415_key_special (int key_in,
                             int x,
                             int y)
{
  struct Engine_OpenGL_GLUT_415_CBData* cb_data_p =
...
void
engine_glut_415_menu (int entry_in)
{
  struct Engine_OpenGL_GLUT_415_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_415_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_415_mouse_button (int button, int state, int x, int y)
{
  struct Engine_OpenGL_GLUT_415_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_415_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
void
engine_glut_415_mouse_move (int x, int y)
{
  struct Engine_OpenGL_GLUT_415_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_415_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
...
engine_glut_415_timer (int v)
{
  ...
  glutTimerFunc (1000 / 30,
                 engine_glut_415_timer,
                 v);
}

void
engine_glut_415_draw (void)
{
  struct Engine_OpenGL_GLUT_415_CBData* cb_data_p =
    static_cast<struct Engine_OpenGL_GLUT_415_CBData*> (glutGetWindowData ());
  ACE_ASSERT (cb_data_p);
  ...
  glProgramUniform2f (cb_data_p->shader.id_, cb_data_p->resolutionLoc,
                      static_cast<GLfloat> (ENGINE_GLUT_415_DEFAULT_WIDTH),
                      static_cast<GLfloat> (ENGINE_GLUT_415_DEFAULT_HEIGHT));
...
#error missing implementation, aborting
  ...
  glProgramUniform3f (cb_data_p->shader.id_, cb_data_p->mouseLoc,
                      static_cast<GLfloat> (cb_data_p->mouseX),
                      static_cast<GLfloat> (Common_GL_Tools::map (cb_data_p->mouseY, 0, ENGINE_GLUT_415_DEFAULT_HEIGHT - 1, ENGINE_GLUT_415_DEFAULT_HEIGHT - 1, 0)),
                      static_cast<GLfloat> (cb_data_p->mouseLMBPressed ? 1.0f : 0.0f));

  glTranslatef (static_cast<GLfloat> (-ENGINE_GLUT_415_DEFAULT_WIDTH / 2.0f),
                static_cast<GLfloat> (-ENGINE_GLUT_415_DEFAULT_HEIGHT / 2.0f),
                0.0f);
...
void
engine_glut_415_idle (void)
{
  glutPostRedisplay ();
}

void
engine_glut_415_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (engine_glut_415_idle);
  else
    glutIdleFunc (NULL);
}
