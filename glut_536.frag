...
float
GetLightAttenuation (float distanceToLight)
{
...
float
sdSmoothUnion (float d1, float d2, float k)
{
...
vec3
Translate (vec3 pos, vec3 translate)
{
  return pos -= translate;
}

float
sdSphere (vec3 p, vec3 origin, float s)
{
...
Material
NormalMaterial (vec3 albedo, int flags)
{
  return Material(albedo, flags);
}

bool
IsLightSource (Material m)
{
  return (m.flags & MATERIAL_IS_LIGHT_SOURCE) != 0;
}

Material
GetMaterial (int materialID, vec3 position)
{
...
float
PlaneIntersection (vec3 rayOrigin, vec3 rayDirection, vec3 planeOrigin, vec3 planeNormal, out vec3 normal)
{ 
  float t = -1.0f;
  normal = planeNormal;
...
float
SphereIntersection (vec3 rayOrigin, vec3 rayDirection, vec3 sphereCenter, float sphereRadius, out vec3 normal)
{
...
void
UpdateIfIntersected (inout float t, float intersectionT, vec3 intersectionNormal, int intersectionMaterialID, out vec3 normal, out int materialID)
{
...
float
IntersectOpaqueScene (vec3 rayOrigin, vec3 rayDirection, out int materialID, out vec3 normal)
{
...
float
QueryVolumetricDistanceField (vec3 pos)
{
...
float
IntersectVolumetric (vec3 rayOrigin, vec3 rayDirection, float maxT)
{
  float precis = 0.5; 
  float t = 0.0f;
...
vec3
Diffuse (vec3 normal, vec3 lightVec, vec3 diffuse)
{
...
float
GetFogDensity (vec3 position, float sdfDistance)
{
...
float
BeerLambert (float absorption, float dist)
{
  return exp(-absorption * dist);
}

float
GetLightVisiblity (vec3 rayOrigin, vec3 rayDirection, float maxT, int maxSteps, float marchSize)
{
  float t = 0.0f;
...
bool
IsColorInsignificant (vec3 color)
{
...
void
CalculateLighting (vec3 position, vec3 normal, vec3 reflectionDirection, Material material, inout vec3 color)
{
  for(int lightIndex = 0; lightIndex < NUM_LIGHTS; lightIndex++)
  {
...
mat3
GetViewMatrix (float xRotationFactor)
{
