...
#define MODEL_ROTATION vec2 (0.3, 0.25)
#define CAMERA_ROTATION vec2 (0.5, 0.5)
...
vec3
intersection (vec3 n, vec3 planeNormal, float planeOffset)
{
...
vec2
icosahedronFaceCoordinates (vec3 p)
{
  vec3 pn = normalize(p);
...
vec3
faceToSphere (vec2 facePoint)
{
  return normalize(facePlane + (uPlane * facePoint.x) + (vPlane * facePoint.y));
}

TriPoints3D
geodesicTriPoints (vec3 p, float subdivisions)
{
...
mat3
sphericalMatrix (float theta, float phi)
{
  float cx = cos(theta);
  float cy = cos(phi);
  float sx = sin(theta);
...
mat3
mouseRotation (bool enable, vec2 xy)
{
  if (enable)
  {
...
HexSpec
newHexSpec (float subdivisions)
{
  return HexSpec (0.05 / subdivisions,
                  0.1  / subdivisions,
                  2.0,
                  2.0,
                  0.005);
...
HexSpec
animHex1 (vec3 hexCenter, float subdivisions)
{
...
HexSpec
animHex2 (vec3 hexCenter, float subdivisions)
{
  HexSpec spec = newHexSpec(subdivisions);
...
HexSpec
animHex3 (vec3 hexCenter, float subdivisions)
{
  HexSpec spec = newHexSpec(subdivisions);
...
float
transitionValues (float a, float b, float c)
{
#ifdef LOOP
#if LOOP == 1
  return a;
...
HexSpec
transitionHexSpecs (HexSpec a, HexSpec b, HexSpec c)
{
...
Model
hexModel (vec3 p, vec3 hexCenter, vec3 edgeA, vec3 edgeB, HexSpec spec)
{
  float d;
...
Model
geodesicModel (vec3 p)
{
  pModIcosahedron (p);

  float subdivisions = transitionValues (animSubdivisions1 (),
                                         animSubdivisions2 (),
                                         animSubdivisions3 ());
  ...
  spec = transitionHexSpecs (animHex1 (points.b, subdivisions),
                             animHex2 (points.b, subdivisions),
                             animHex3 (points.b, subdivisions));
  part = hexModel(p, points.b, edgeAB, edgeBC, spec);
  model = part;

  spec = transitionHexSpecs (animHex1 (points.c, subdivisions),
  ...
  spec = transitionHexSpecs (animHex1 (points.a, subdivisions),
...
vec3
doLighting (Model model, vec3 pos, vec3 nor, vec3 ref, vec3 rd)
{
